...
const baseRatesCallData: object = { symbols: "", multiplier: 0 };

export const RatesCallData = {
  encode(message: RatesCallData, writer: Writer = Writer.create()): Writer {
  ...
  decode(input: Reader | Uint8Array, length?: number): RatesCallData {
    ...
    const message = { ...baseRatesCallData } as RatesCallData;
    message.symbols = [];
...
const baseRatesResult: object = { rates: 0 };

export const RatesResult = {
  encode(message: RatesResult, writer: Writer = Writer.create()): Writer {
  ...
  decode(input: Reader | Uint8Array, length?: number): RatesResult {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseRatesResult } as RatesResult;
    message.rates = [];
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
