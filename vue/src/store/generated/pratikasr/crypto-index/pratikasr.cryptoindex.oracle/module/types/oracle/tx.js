...
export const MsgRatesData = {
    ...
    decode(input, length) {
        ...
        const message = { ...baseMsgRatesData };
        message.feeLimit = [];
...
const baseMsgRatesDataResponse = {};
export const MsgRatesDataResponse = {
    ...
    decode(input, length) {
        const reader = input instanceof Uint8Array ? new Reader(input) : input;
        let end = length === undefined ? reader.len : reader.pos + length;
        const message = { ...baseMsgRatesDataResponse };
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                default:
...
export class MsgClientImpl {
    ...
    RatesData(request) {
        ...
        const promise = this.rpc.request("pratikasr.cryptoindex.oracle.Msg", "RatesData", data);
        return promise.then((data) => MsgRatesDataResponse.decode(new Reader(data)));
    }
}
var globalThis = (() => {
