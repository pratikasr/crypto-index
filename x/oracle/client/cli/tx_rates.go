...
// CmdRequestRatesData creates and broadcast a Rates request transaction
func CmdRequestRatesData() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "rates-data [oracle-script-id] [requested-validator-count] [sufficient-validator-count]",
		Short: "Make a new Rates query request via an existing BandChain oracle script",
