...
func (am AppModule) handleOraclePacket(
	...
	case types.RatesClientIDKey:
		var ratesResult types.RatesResult
		if err := obi.Decode(modulePacketData.Result, &ratesResult); err != nil {
			ack = channeltypes.NewErrorAcknowledgement(err.Error())
			return ack, sdkerrors.Wrap(sdkerrors.ErrUnknownRequest,
				"cannot decode the rates received packet")
		}
		am.keeper.SetRatesResult(ctx, types.OracleRequestID(modulePacketData.RequestID), ratesResult)

		// TODO: Rates oracle data reception logic
...
func (am AppModule) handleOracleAcknowledgment(
	...
	case *channeltypes.Acknowledgement_Result:
		...
		case types.RatesClientIDKey:
			var ratesData types.RatesCallData
			if err = obi.Decode(data.GetCalldata(), &ratesData); err != nil {
				return nil, sdkerrors.Wrap(err,
					"cannot decode the rates oracle acknowledgment packet")
			}
			am.keeper.SetLastRatesID(ctx, requestID)
